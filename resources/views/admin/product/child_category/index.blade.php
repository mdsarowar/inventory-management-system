    ...
    <div class="content">
        <div class="page-header">
            <div class="page-title">
                <h4>Product Child Category list</h4>
                <h6>View/Search product Child Category</h6>
            </div>
            <div class="page-btn">
                <a href="{{route('child_category.create')}}" class="btn btn-added">
                    <img src="{{asset('/')}}admin/assets/img/icons/plus.svg"  class="me-1" alt="img">Add Child Category
                </a>
            </div>
        </div>
...
{{--                </div>--}}
                ...
                <div class="table-responsive">
                    <table class="table  datanew">
                        ...
                        <tbody>
                        @foreach($child_categoris as $child_cat)
                            <tr>
                                <td>
                                    <label class="checkboxs">
                                        <input type="checkbox">
                                ...
                                <td class="productimgname">
                                    <a href="javascript:void(0);" class="product-img">
                                        <img src="{{asset($child_cat->image)}}" alt="product">
                                    </a>
                                ...
                                <td>{{$child_cat->created_by}}</td>
                                <td>{{ $child_cat->status == 1? 'Active':'Inactive' }}</td>
                                <td>
                                    @can('update brand')
                                        <a class="me-3" href="{{route('child_category.edit',$child_cat->id)}}">
                                            <img src="{{asset('/')}}admin/assets/img/icons/edit.svg" alt="img">
                                        </a>
                                    @endcan
                                    @can('delete brand')
                                        <form action="{{route('child_category.destroy',$child_cat->id)}}" method="POST" class="sr-dl" >
                                            @csrf
                                            @method('delete')
